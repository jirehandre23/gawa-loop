  ...
  return (
    <>
      ...
      <style>{`
        ...
        .social-icon:hover { color: #16a34a !important; }
      ...
      <main dir={isRTL ? "rtl" : "ltr"} className="min-h-screen bg-white text-slate-900"
        ...
        <section style={{ background:"#0a2e1a", padding:"72px 24px" }}>
          <div style={{ maxWidth:"820px", margin:"0 auto", textAlign:"center" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"20px" }}>
              ...
              <div style={{ background:"rgba(255,255,255,0.08)", borderRadius:"20px", padding:"36px 24px" }}>
                <p style={{ margin:"0 0 8px", fontSize:"56px", fontWeight:900, color:"#4ade80", lineHeight:1 }}>{statsLoaded ? liveStats.co2_saved_lbs.toLocaleString() : "—"}</p>
                <p style={{ margin:0, fontSize:"16px", color:"#a3c9b0", fontWeight:600 }}>🌱 {locale==="ar"?"رطل CO₂e تم توفيرها":locale==="fr"?"lbs CO₂e économisées":locale==="es"?"lbs CO₂e ahorradas":locale==="pt"?"lbs CO₂e salvas":"lbs CO₂e saved"}</p>
              </div>
            </div>
          </div>
        </section>
            ...
            <div className="grid items-center gap-12 md:grid-cols-2">
              <div>
                ...
                <h2 className="mb-6 text-4xl font-extrabold leading-tight">
                  {locale==="ar"?<>{"حوّل الطعام الفائض إلى"}<br/><span className="text-green-400">{"حسن نية مجتمعي"}</span></>:locale==="fr"?<>{"Transformez les surplus en"}<br/><span className="text-green-400">{"soutien communautaire"}</span></>:locale==="es"?<>{"Convierte el excedente en"}<br/><span className="text-green-400">{"buena voluntad comunitaria"}</span></>:locale==="pt"?<>{"Transforme excedentes em"}<br/><span className="text-green-400">{"boa vontade comunitária"}</span></>:<>{"Turn surplus food into"}<br/><span className="text-green-400">{"community goodwill"}</span></>}
                </h2>
                <p className="mb-8 text-lg text-slate-300 leading-relaxed">{T.mission_desc}</p>
                <div className="mb-8 space-y-4">
                  {[T.step1_desc, T.step2_desc, T.step3_desc, locale==="ar"?"بناء سمعة كشركة مجتمعية":locale==="fr"?"Construisez votre réputation":locale==="es"?"Construye reputación comunitaria":locale==="pt"?"Construa reputação comunitária":"Build reputation as a community-first business"].map((item, i) => (
        ...
        <section id="faq" style={{ background:"#f8fafc", padding:"80px 24px" }}>
          <div style={{ maxWidth:"720px", margin:"0 auto" }}>
            ...
            <p style={{ margin:"0 0 48px", textAlign:"center", color:"#64748b", fontSize:"15px" }}>{locale==="ar"?"كل ما تحتاج معرفته":locale==="fr"?"Tout ce que vous devez savoir":locale==="es"?"Todo lo que necesitas saber":locale==="pt"?"Tudo o que você precisa saber":"Everything you need to know about GAWA Loop"}</p>
            <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
              {faqs.map((faq, i) => (
                <div key={i} style={{ background:"#fff", borderRadius:"16px", border:"1px solid #e2e8f0", overflow:"hidden" }}>
                  ...
                  {openFaq === i && <div style={{ padding:"0 24px 20px" }}><p style={{ margin:0, color:"#475569", lineHeight:1.7, fontSize:"14px" }}>{faq.a}</p></div>}
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="signin" style={{ background:"#f0fdf4", padding:"80px 24px", borderTop:"1px solid #bbf7d0" }}>
          <div style={{ maxWidth:"480px", margin:"0 auto" }}>
            ...
            <h2 style={{ margin:"0 0 8px", fontSize:"32px", fontWeight:800, color:"#0a2e1a", textAlign:"center" }}>
              {signinMode === "signin" ? (locale==="ar"?"سجّل الدخول":locale==="fr"?"Se connecter":locale==="es"?"Iniciar sesión":locale==="pt"?"Entrar":"Sign in to your account") : (locale==="ar"?"إنشاء حساب مجاني":locale==="fr"?"Créer un compte":locale==="es"?"Crear cuenta":locale==="pt"?"Criar conta":"Create a free account")}
            </h2>
            <p style={{ margin:"0 0 32px", color:"#4b7c5e", textAlign:"center", fontSize:"15px" }}>
              {signinMode === "signin" ? (locale==="ar"?"الوصول إلى حجوزاتك":locale==="fr"?"Accédez à vos réservations":locale==="es"?"Accede a tus reservas":locale==="pt"?"Acesse suas reservas":"Access your reservations and order history.") : (locale==="ar"?"انضم للحصول على طعام مجاني":locale==="fr"?"Rejoignez-nous pour de la nourriture gratuite":locale==="es"?"Únete para obtener comida gratis":locale==="pt"?"Junte-se para comida grátis":"Join GAWA Loop to start claiming free food.")}
            </p>
              ...
              {user ? (
                <div style={{ textAlign:"center" }}>
                  ...
                  <div style={{ display:"flex", gap:"12px", justifyContent:"center", flexWrap:"wrap" }}>
                    <a href="/customer/profile" style={{ background:"#16a34a", color:"#fff", padding:"12px 24px", borderRadius:"10px", textDecoration:"none", fontWeight:700, fontSize:"14px" }}>{locale==="ar"?"حسابي":locale==="fr"?"Mon Compte":locale==="es"?"Mi Cuenta":locale==="pt"?"Minha Conta":"My Account"}</a>
                    ...
                    <button onClick={async () => { await supabase.auth.signOut(); setUser(null); }} style={{ background:"none", border:"none", color:"#9ca3af", fontSize:"13px", cursor:"pointer" }}>{locale==="ar"?"خروج":locale==="fr"?"Déconnexion":locale==="es"?"Cerrar sesión":locale==="pt"?"Sair":"Sign Out"}</button>
                  </div>
                </div>
              ) : (
                <form onSubmit={handleCustomerAuth}>
                  ...
                  <div style={{ marginBottom:"14px" }}>
                    <label style={{ display:"block", fontSize:"13px", fontWeight:600, color:"#374151", marginBottom:"6px" }}>{locale==="ar"?"البريد الإلكتروني":locale==="fr"?"Adresse email":locale==="es"?"Correo electrónico":locale==="pt"?"Email":"Email Address"}</label>
                  ...
                  <div style={{ marginBottom: signinMode === "signup" ? "16px" : "20px" }}>
                    <label style={{ display:"block", fontSize:"13px", fontWeight:600, color:"#374151", marginBottom:"6px" }}>{locale==="ar"?"كلمة المرور":locale==="fr"?"Mot de passe":locale==="es"?"Contraseña":locale==="pt"?"Senha":"Password"}</label>
                  ...
                  <button type="submit" disabled={signinLoading || (signinMode === "signup" && !termsAccepted)}
                    ...
                    {signinLoading ? "..." : signinMode === "signin" ? (locale==="ar"?"دخول":locale==="fr"?"Se connecter":locale==="es"?"Iniciar sesión":locale==="pt"?"Entrar":"Sign In") : (locale==="ar"?"إنشاء حساب":locale==="fr"?"Créer le compte":locale==="es"?"Crear cuenta":locale==="pt"?"Criar conta":"Create Account")}
                  </button>
                  <p style={{ textAlign:"center", fontSize:"13px", color:"#6b7280", margin:0 }}>
                    {signinMode === "signin" ? (locale==="ar"?"ليس لديك حساب؟":locale==="fr"?"Pas de compte ?":locale==="es"?"¿Sin cuenta?":locale==="pt"?"Sem conta?":"No account yet?") : (locale==="ar"?"لديك حساب؟":locale==="fr"?"Déjà un compte ?":locale==="es"?"¿Ya tienes cuenta?":locale==="pt"?"Já tem conta?":"Already have an account?")}{" "}
                    <button type="button" onClick={() => { setSigninMode(signinMode === "signin" ? "signup" : "signin"); setSigninError(""); setTermsAccepted(false); }} style={{ background:"none", border:"none", color:"#16a34a", fontWeight:700, cursor:"pointer", fontSize:"13px", padding:0 }}>
                      {signinMode === "signin" ? (locale==="ar"?"إنشاء حساب مجاني":locale==="fr"?"Créer un compte":locale==="es"?"Crear cuenta":locale==="pt"?"Criar conta":"Create one free") : (locale==="ar"?"تسجيل الدخول":locale==="fr"?"Se connecter":locale==="es"?"Iniciar sesión":locale==="pt"?"Entrar":"Sign in")}
                    </button>
                  </p>
                </form>
              )}
            ...
            <p style={{ textAlign:"center", marginTop:"16px", fontSize:"12px", color:"#9ca3af" }}>
              ...
              <a href="/business/login" style={{ color:"#16a34a", fontWeight:600, textDecoration:"none" }}>{locale==="ar"?"دخول الشركات ←":locale==="fr"?"Connexion entreprise →":locale==="es"?"Login empresas →":locale==="pt"?"Login empresas →":"Business login →"}</a>
            </p>
          </div>
        </section>
        ...
        <section id="contact" style={{ background:"#fff", padding:"80px 24px", borderTop:"1px solid #e5e7eb" }}>
          <div style={{ maxWidth:"560px", margin:"0 auto" }}>
            ...
            <h2 style={{ margin:"0 0 8px", fontSize:"32px", fontWeight:800, color:"#0a2e1a", textAlign:"center" }}>{locale==="ar"?"اتصل بنا":locale==="fr"?"Nous contacter":locale==="es"?"Contáctanos":locale==="pt"?"Contate-nos":"Contact Us"}</h2>
            <p style={{ margin:"0 0 32px", color:"#64748b", textAlign:"center", fontSize:"15px" }}>{locale==="ar"?"لديك سؤال أو فكرة؟ نحن نحب أن نسمع منك.":locale==="fr"?"Une question ou une idée ? Nous serions ravis de vous lire.":locale==="es"?"¿Tienes una pregunta? Nos encantaría saber de ti.":locale==="pt"?"Tem uma pergunta? Adoraríamos ouvir de você.":"Have a question, partnership idea, or just want to say hi? We'd love to hear from you."}</p>
            <div style={{ background:"#f9fafb", borderRadius:"20px", padding:"32px", border:"1px solid #e5e7eb" }}>
              {contactDone ? (
                <div style={{ textAlign:"center", padding:"24px 0" }}>
                  <div style={{ fontSize:"48px", marginBottom:"12px" }}>✅</div>
                  <h3 style={{ margin:"0 0 8px", fontSize:"20px", fontWeight:800, color:"#0a2e1a" }}>{locale==="ar"?"تم الإرسال!":locale==="fr"?"Message envoyé !":locale==="es"?"¡Mensaje enviado!":locale==="pt"?"Mensagem enviada!":"Message sent!"}</h3>
                  <p style={{ margin:"0 0 20px", color:"#6b7280", fontSize:"14px" }}>{locale==="ar"?"سنرد عليك خلال 24-48 ساعة.":locale==="fr"?"Nous vous répondrons sous 24–48h.":locale==="es"?"Te responderemos en 24–48h.":locale==="pt"?"Responderemos em 24–48h.":"We'll get back to you within 24–48 hours."}</p>
                  <button onClick={() => setContactDone(false)} style={{ background:"#f3f4f6", border:"1px solid #e5e7eb", padding:"10px 20px", borderRadius:"8px", cursor:"pointer", fontWeight:600, fontSize:"14px", color:"#374151" }}>{locale==="ar"?"إرسال رسالة أخرى":locale==="fr"?"Envoyer un autre message":locale==="es"?"Enviar otro mensaje":locale==="pt"?"Enviar outra mensagem":"Send another message"}</button>
                </div>
              ) : (
                <form onSubmit={handleContact}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"14px", marginBottom:"14px" }}>
                    <div>
                      <label style={{ display:"block", fontSize:"13px", fontWeight:600, color:"#374151", marginBottom:"6px" }}>{locale==="ar"?"الاسم *":locale==="fr"?"Votre nom *":locale==="es"?"Tu nombre *":locale==="pt"?"Seu nome *":"Your Name *"}</label>
                      <input style={inp} required value={contactName} onChange={e => setContactName(e.target.value)} placeholder="Alex Johnson" />
                    </div>
                    <div>
                      <label style={{ display:"block", fontSize:"13px", fontWeight:600, color:"#374151", marginBottom:"6px" }}>{locale==="ar"?"البريد الإلكتروني *":locale==="fr"?"Email *":locale==="es"?"Email *":locale==="pt"?"Email *":"Email *"}</label>
                  ...
                  <div style={{ marginBottom:"20px" }}>
                    <label style={{ display:"block", fontSize:"13px", fontWeight:600, color:"#374151", marginBottom:"6px" }}>{locale==="ar"?"الرسالة *":locale==="fr"?"Message *":locale==="es"?"Mensaje *":locale==="pt"?"Mensagem *":"Message *"}</label>
                    <textarea style={{ ...inp, height:"120px", resize:"vertical" }} required value={contactMsg} onChange={e => setContactMsg(e.target.value)} placeholder={locale==="ar"?"اكتب رسالتك هنا...":locale==="fr"?"Votre message...":locale==="es"?"Tu mensaje...":locale==="pt"?"Sua mensagem...":"Tell us what's on your mind..."} />
                  </div>
                  <button type="submit" disabled={contactSending} style={{ width:"100%", background:contactSending?"#9ca3af":"#0a2e1a", color:"#fff", border:"none", padding:"13px", borderRadius:"10px", cursor:contactSending?"not-allowed":"pointer", fontSize:"15px", fontWeight:700 }}>
                  ...
                  <p style={{ textAlign:"center", marginTop:"14px", fontSize:"12px", color:"#9ca3af" }}>{locale==="ar"?"أو راسلنا مباشرة:":locale==="fr"?"Ou par email :":locale==="es"?"O escríbenos:":locale==="pt"?"Ou por email:":"Or email directly:"}{" "}<a href="mailto:admin@gawaloop.com" style={{ color:"#16a34a", fontWeight:600, textDecoration:"none" }}>admin@gawaloop.com</a></p>
                </form>
              )}
            </div>
          </div>
        ...
        <section className="bg-gradient-to-br from-green-500 to-emerald-600">
          <div className="mx-auto max-w-4xl px-6 py-20 text-center">
            ...
            <p className="mb-10 text-lg text-green-100">{locale==="ar"?"انضم لمجتمعك. أقل من دقيقة.":locale==="fr"?"Rejoignez votre communauté en moins d'une minute.":locale==="es"?"Únete a tu comunidad en menos de un minuto.":locale==="pt"?"Junte-se à sua comunidade em menos de um minuto.":"Join your community. Find free food or share yours — less than a minute."}</p>
        ...
        <footer className="border-t border-slate-100 bg-white">
          <div className="mx-auto max-w-6xl px-6 py-10">
            <div className="flex flex-col items-center justify-between gap-6 md:flex-row">
              <a href="/" className="flex items-center gap-3"><Image src="/gawa-logo-green.png" width={32} height={32} alt="GAWA Loop" style={{ objectFit:"contain" }} /><span className="font-bold text-slate-900">GAWA Loop</span></a>
              <div className="flex flex-wrap justify-center gap-6 text-sm text-slate-500">
                ...
                <a href="mailto:admin@gawaloop.com" className="hover:text-green-600 transition">{locale==="ar"?"اتصل بنا":locale==="fr"?"Contact":locale==="es"?"Contacto":locale==="pt"?"Contato":"Contact Us"}</a>
              </div>
              {/* ── SOCIAL ICONS ── */}
              <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                <a href="https://www.youtube.com/@GawaLoop" target="_blank" rel="noopener noreferrer" className="social-icon" style={{ color: "#9ca3af", transition: "color 0.15s" }} aria-label="YouTube">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                </a>
                <a href="https://www.facebook.com/people/GawaLoop/61567703090095/" target="_blank" rel="noopener noreferrer" className="social-icon" style={{ color: "#9ca3af", transition: "color 0.15s" }} aria-label="Facebook">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
                <a href="https://www.instagram.com/gawaloop/" target="_blank" rel="noopener noreferrer" className="social-icon" style={{ color: "#9ca3af", transition: "color 0.15s" }} aria-label="Instagram">
                ...
                <a href="https://www.tiktok.com/@gawaloop_?is_from_webapp=1&sender_device=pc" target="_blank" rel="noopener noreferrer" className="social-icon" style={{ color: "#9ca3af", transition: "color 0.15s" }} aria-label="TikTok">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.18 8.18 0 0 0 4.78 1.52V6.75a4.85 4.85 0 0 1-1.01-.06z"/></svg>
                </a>
              </div>
            </div>
            <div className="text-center text-sm text-slate-400" style={{ marginTop: "16px" }}>© 2026 GAWA Loop · Free food. Less waste. Real impact.</div>
          </div>
        </footer>
