...
export async function POST(req: NextRequest) {
  ...
  await supabase
    .from("listings")
    .update({ status: "RESERVED", reserved_until: reservedUntil, claim_code: confirmationCode })
    .eq("id", listingId);

  // ✅ RESPOND TO USER IMMEDIATELY — don't wait for emails
  const response = NextResponse.json({ success: true, code: confirmationCode, claimId: claim.id });
  ...
  // Send all emails in background — no await, no blocking
  Promise.all([
    // ✅ Customer confirmation
    sendEmail(
      email,
      `✅ Reserved — ${listing.food_name} at ${listing.business_name}`,
      emailWrapper(`
        ...
        <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:12px 16px;margin-bottom:20px;">
          <p style="margin:0;font-size:13px;color:#92400e;">📬 <b>Don't see this email?</b> Check your spam or junk folder and mark it as "Not spam" to receive future messages instantly.</p>
        </div>
    ...
    supabase.from("businesses").select("email").eq("name", listing.business_name).single().then(({ data: biz }) => {
      if (biz?.email) {
        return sendEmail(
          biz.email,
          `🔔 New Reservation — ${listing.food_name}`,
          emailWrapper(`
    ...
    sendEmail(
      "jireh@gawaloop.com",
      `🔔 New Claim — ${listing.food_name} at ${listing.business_name}`,
      emailWrapper(`
  ...
  ]).catch(() => {}); // silently ignore email errors — never block the user
