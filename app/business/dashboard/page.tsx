...
export default function BusinessDashboard() {
  ...
  async function handleCancelListing(id: string) {
    ...
    if (data.success) {
      ...
      if (data.notified > 0) alert(`Listing cancelled. ${data.notified} customer(s) notified by email.`);
    }
  }
  ...
  async function handlePost(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("listings").insert({
      ...
      food_name: form.food_name, category: form.category, quantity: form.quantity,
      allergy_note: form.allergy_note || null,
      estimated_value: form.estimated_value ? Number(form.estimated_value) : null,
      note: form.note || null, status: "AVAILABLE",
      expires_at: expiresAt, claim_hold_minutes: Number(form.claim_hold),
    });
    if (error) { setPostMsg("Error posting. Please try again."); }
    else { setPostMsg("Food posted successfully! ✅"); setForm(EMPTY_FORM); setShowForm(false); loadDashboard(adminView); }
    setPosting(false);
  }
            ...
            <form onSubmit={handlePost}>
              <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:"16px"}}>
                <div style={{gridColumn:"1/-1"}}>
                  ...
                  <input style={{...inp, background:"#f9fafb", color:"#374151"}} value={businessName||""} disabled/>
                </div>
                <div style={{gridColumn:"1/-1"}}>
                  <label style={lbl}>Pickup Address</label>
                  <input style={{...inp, background:"#f9fafb", color:"#374151"}} value={businessAddress||""} disabled/>
                </div>
                <div style={{gridColumn:"1/-1"}}>
                ...
                <div style={{gridColumn:"1/-1"}}>
                  <label style={lbl}>{T.dash_allergy}</label>
                ...
                <div>
                  ...
                  <select style={{...inp,cursor:"pointer"}} value={form.active_hours} onChange={e => setForm(f=>({...f,active_hours:e.target.value}))}>
                    <option value="0.5">30 minutes</option><option value="1">1 hour</option>
                    <option value="2">2 hours</option><option value="4">4 hours</option><option value="8">8 hours</option>
                  </select>
                </div>
                <div style={{gridColumn:"1/-1"}}>
              ...
              <button type="submit" disabled={posting}
                ...
                {posting ? "Posting..." : T.dash_post_btn}
              </button>
            </form>
          </div>
        )}
          ...
          return (
            <div key={listing.id} style={{background:"#fff", border:"1px solid #e5e7eb", borderRadius:"16px", padding:"24px", marginBottom:"16px", opacity:isTerminal?0.85:1}}>
              ...
              <div style={{fontSize:"14px", color:"#1f2937", lineHeight:"1.9"}}>
                ...
                <p style={{margin:"2px 0"}}><b>{T.allergy}:</b> {listing.allergy_note||"None"}</p>
                <p style={{margin:"2px 0"}}><b>{T.value}:</b> ${Number(listing.estimated_value||0).toFixed(2)}</p>
