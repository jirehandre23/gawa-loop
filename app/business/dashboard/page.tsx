...
export default function BusinessDashboard() {
  ...
  async function loadDashboard(adminTarget?: string | null) {
    ...
    if (admin) {
      ...
      const unique = [...new Set((data || []).map((b: any) => b.business_name).filter(Boolean))] as string[];
      setAllBizNames(unique);
    }

    const { data: biz } = await supabase.from("businesses").select("name, address").eq("email", email).single();
    const bName = adminTarget ?? (admin ? null : biz?.name) ?? biz?.name ?? null;
    setBusiness(bName);
  ...
  const yearlyListings = listings.filter(l => new Date(l.created_at).getFullYear() === thisYear);

  const monthlyValue  = monthlyListings.reduce((s, l) => s + Number(l.estimated_value || 0), 0);
  const yearlyValue   = yearlyListings.reduce((s, l) => s + Number(l.estimated_value || 0), 0);
  const monthPickups  = monthlyListings.filter(l => l.status === "PICKED_UP").length;
  const yearPickups   = yearlyListings.filter(l => l.status === "PICKED_UP").length;
      ...
      <div style={{ maxWidth:"780px", margin:"0 auto" }}>
            ...
            <form onSubmit={handlePost}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"16px" }}>
                ...
                <div>
                  ...
                  <select style={sel} value={form.active_hours} onChange={e => setForm(f => ({ ...f, active_hours: e.target.value }))}>
                ...
                <div style={{ gridColumn:"1/-1" }}>
                  ...
                  <select style={sel} value={form.claim_hold} onChange={e => setForm(f => ({ ...f, claim_hold: e.target.value }))}>
        ...
        <div style={{ background:"#fff", border:"1px solid #e5e7eb", borderRadius:"16px", padding:"24px 28px", marginBottom:"24px" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"16px" }}>
            <h2 style={{ margin:0, fontSize:"17px", fontWeight:800, color:"#0a2e1a" }}>📊 Impact Summary</h2>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px" }}>
            {[
              { label:`This Month (${now.toLocaleString("default",{month:"long"})})`, items:[
                { k:"Listings posted", v: monthlyListings.length },
                { k:"Pickups completed", v: monthPickups },
                { k:"Estimated value", v: `$${monthlyValue.toFixed(2)}` },
              ]},
              { label:`This Year (${thisYear})`, items:[
                { k:"Listings posted", v: yearlyListings.length },
                { k:"Pickups completed", v: yearPickups },
                { k:"Estimated value", v: `$${yearlyValue.toFixed(2)}` },
              ]},
            ].map(section => (
              <div key={section.label} style={{ background:"#f9fafb", borderRadius:"12px", padding:"16px 20px", border:"1px solid #e5e7eb" }}>
                ...
                {section.items.map(item => (
                  <div key={item.k} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"4px 0", borderBottom:"1px solid #f0f0f0" }}>
          ...
          <p style={{ margin:"12px 0 0", fontSize:"12px", color:"#9ca3af" }}>This summary is for recordkeeping and operational reporting.</p>
        </div>
        ...
        {listings.length === 0 ? (
          <div style={{ background:"#fff", border:"1px solid #e5e7eb", borderRadius:"12px", padding:"40px", textAlign:"center" }}>
            <p style={{ color:"#6b7280", marginBottom:"16px" }}>No listings yet. Post your first one!</p>
          ...
          const pickedUpClaim = listing.claims?.find(c => c.status === "active");
          ...
          return (
            <div key={listing.id} style={{ background:"#fff", border:"1px solid #e5e7eb", borderRadius:"16px", padding:"24px", marginBottom:"16px", opacity: isTerminal ? 0.85 : 1 }}>
              ...
              <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:"14px", gap:"12px" }}>
                ...
                <span style={{ background:sc.bg, color:sc.text, fontSize:"12px", fontWeight:700, padding:"5px 14px", borderRadius:"20px", flexShrink:0, whiteSpace:"nowrap" }}>
                  {listing.status}
                </span>
              </div>

              {/* Details — always fully readable */}
              <div style={{ fontSize:"14px", color:"#374151", lineHeight:"1.85" }}>
              ...
              {/* PICKED_UP — show who picked it up, fully readable */}
              {listing.status === "PICKED_UP" && pickedUpClaim && (
                <div style={{ background:"#f5f3ff", border:"1.5px solid #ddd6fe", borderRadius:"12px", padding:"16px 20px", marginTop:"16px" }}>
                  ...
                  <div style={{ fontSize:"14px", color:"#3b0764", lineHeight:"1.8" }}>
                    <p style={{ margin:"2px 0" }}><b>Name:</b> {pickedUpClaim.first_name}</p>
                    <p style={{ margin:"2px 0" }}><b>Email:</b> {pickedUpClaim.email}</p>
                    <p style={{ margin:"2px 0" }}><b>Phone:</b> {pickedUpClaim.phone || "Not provided"}</p>
                    <p style={{ margin:"2px 0" }}><b>Confirmation code:</b> <span style={{ fontWeight:800, fontSize:"16px", color:"#7c3aed" }}>{pickedUpClaim.confirmation_code}</span></p>
                  </div>
                </div>
              )}
